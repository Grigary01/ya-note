...
def test_author_can_delete_note(author_client, slug_for_page):
    url = reverse('notes:delete', args=slug_for_page)
    response = author_client.post(url)
    assertRedirects(response, reverse('notes:success'))
    assert Note.objects.count() == 0


def test_other_user_cant_delete_note(not_author_client, slug_for_page):
    url = reverse('notes:delete', args=slug_for_page)
